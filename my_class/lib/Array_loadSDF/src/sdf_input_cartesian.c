...
Array Array_sdf_read_plain_mesh(sdf_file_t *h){
	...
	for(i=0;i<b->ndims;i++){
		//printf("%d\n",b->local_dims[i]);
		if(row < b->local_dims[i]){
			row = b->local_dims[i];
		}
	}
	
	array = Array_create(2,row-1,b->ndims);
		...
		if (b->ndims > n) {
			...
			switch(b->datatype){
			  case SDF_DATATYPE_REAL8:
				a.d = b->grids[n];
				for(i=0;i<b->local_dims[n]-1;i++){Array_set(array,a.d[i],n,i);}
				break;
			  case SDF_DATATYPE_REAL4:
				a.f = b->grids[n];
				for(i=0;i<b->local_dims[n]-1;i++){Array_set(array,(double)(a.f[i]),n,i);}
				break;
			  default:
				break;
			}
...
Array Array_sdf_read_plain_variable(sdf_file_t *h){
	...
	if(b->datatype_out == SDF_DATATYPE_REAL8){
        ...
        //for (n=0; n<b->ndims; n++) SDF_DPRNT("%i ",b->local_dims[n]);
		//SDF_DPRNT("\n  ");
		//printf("local->dims[0] : %d\n" ,b->local_dims[0]);
		//printf("local->dims[1] : %d\n" ,b->local_dims[1]);
		//printf("local->dims[2] : %d\n" ,b->local_dims[2]);
		//SDF_DPRNTar2(b->data, b->nelements_local,b->local_dims[0]);
	}
	*/
    b->done_data = 1;
    return array;
