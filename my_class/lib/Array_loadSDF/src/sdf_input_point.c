...
Array Array_sdf_read_point_variable(sdf_file_t *h)
{
	...
	n = b->nelements_local;
	array = Array_create(1,n);
	
	switch(b->datatype_out){
	  case SDF_DATATYPE_REAL8:
		a.d = b->data;
		for(i=0;i<n;i++){Array_set(array,a.d[i],i);}
		break;
	  case SDF_DATATYPE_REAL4:
		a.f = b->data;
		for(i=0;i<n;i++){Array_set(array,(double)(a.f[i]),i);}
		break;
	  default:
		break;
	}
