...
int main(int argc, char *argv[]){
	...
	if(comdef){
		if((arglist=parsefunc(comdef)) == NULL){printf("(?_?) Pardon me?\n"); goto visiter_finish;}
	...
	while(1){
		...
		if(!strcmp(buf,"bye")){puts("(*'-')/");deallocate(buf); break;}
		if(!strcmp(buf,"hello") || !strcmp(buf,"hi")){puts( "('-'*)/ hi" );goto loopend;}
		...
		if((arglist=parsefunc(buf)) == NULL){printf("(?_?) pardon?\n");goto loopend;}
