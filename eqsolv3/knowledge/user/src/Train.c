...
Expr Train(Expr expr){
	...
	for(e=expr->child;e;e=e->next){
		if(e->symbol->id == id_String){
			if(!strcmp(e->data.string,"Accident")){ACCIDENT=1;}
			if(!strcmp(e->data.string,"Fly")){FLY=1;}
			if(!strcmp(e->data.string,"Logo")){LOGO=1;}
			if(!strcmp(e->data.string,"C51")){C51=1;}
		}
		if(e->symbol->id == id_Integer && !(e->flag & ef_full)){
			speed = Integer_toInt(e);
		}
	}
	initscr();
