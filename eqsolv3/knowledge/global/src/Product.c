#include "knowledge.h"
Expr Product_replace(Expr expr, Expr local, Expr rhs){
	//puts("replace");
	Expr root,e;
	if(!expr){return expr;}
	...
	if(expr->child){
		for(e=expr->child;e;e=e->next){
			Expr_appendChild(root,Product_replace(e,local,rhs));
		}
	}
	return root;
}
Expr Product_replaceInt(Expr expr, Expr local, int k){
	//puts("replaceInt");
	Expr root,e;
	if(!expr){return expr;}
	if(expr->symbol == local->symbol){
	...
	if(expr->child){
		for(e=expr->child;e;e=e->next){
			Expr_appendChild(root,Product_replaceInt(e,local,k));
		}
	}
	return root;
}
Expr Product_iterate(Expr expr, Expr local,int imin,int imax,int di){
	Expr root;
	int i;
	//puts("replaceIterate");
	if(!expr){return expr;}
	root = Expr_create(id_Times);
	for(i=imin;i<=imax;i+=di){
		Expr_appendChild(root,Evaluate(Product_replaceInt(expr,local,i)));
	}
	return root;
}
Expr Product(Expr expr){
	...
	if(spec->child->next->symbol->id==id_List){
		...
		for(e=spec->child->next->child;e;e=e->next){
			Expr_appendChild(rtn,Evaluate(Product_replace(main,local,e)));
		}
		goto end;
	}
	if(slen==2){
	...
	main->next = NULL;
	rtn = Product_iterate(main,local,imin,imax,di);
  end:
	main->next = spec;
	Expr_deleteRoot(expr);
	return Evaluate(rtn);
  itraw:
	fprintf(stderr,"Product::itraw: Raw object ");
	FullForm_mod2(stderr,local);
	fprintf(stderr," cannot be used as an iterator\n");
	return expr;
  itwrite:
	fprintf(stderr,"Product::write: Tag ");
	FullForm_mod2(stderr,local);
	fprintf(stderr," in ");
	...
	return expr;
  itform:
	fprintf(stderr,"Product::itform: Argument ");
	FullForm_mod2(stderr,spec);
	fprintf(stderr," does not have the correct form for an iterator.\n");
	return expr;
  iterb:
	fprintf(stderr,"Product::iterb: Iterator ");
	FullForm_mod2(stderr,spec);
