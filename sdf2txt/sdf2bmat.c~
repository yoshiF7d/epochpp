...
void usage(int err)
{
    fprintf(stderr, "usage: sdf2bmat [options] <sdf_filename>\n");
    fprintf(stderr, "\noptions:\n\
...
char *parse_args(int *argc, char ***argv)
{
	...
	while ((c = getopt_long(*argc, *argv, "hncsmiv:f:", longopts, NULL)) != -1) {
        switch (c) {
        case 'h':
            usage(0);
            break;
        ...
        case 'v':
			...
			break;
	 case 'f':
	     strncpy(fileout,optarg,strlen(optarg)+1);
        case 'm':
            use_mmap = 1;
            break;
        case 'i':
