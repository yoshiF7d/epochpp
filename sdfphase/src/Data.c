...
Data Data_histogram2(Data data, double min, double max, int binc){
	int i,j;
	double *weight = allocate(binc*sizeof(double));
	double dx = (max-min)/binc;
	double coeff = 1.0/dx;
...
Data Data_histogram2D2(Data data, double min1, double min2, double max1, double max2, int binc1, int binc2){
	int i,j,k, binc = binc1*binc2;
	double *weight = allocate(binc*sizeof(double));
	double dx = (max1-min1)/binc1;
	double dy = (max2-min2)/binc2;
	...
	for(i=0;i<binc;i++){data2->elem[i][2] = weight[i]; /*printf("%f\n",data2->elem[i][2]);*/}
	deallocate(weight);
	return data2;
}
