...
Data sdf_read_plain_mesh(sdf_file_t *h){
	...
	col= 0;
	for(n = 0; n < 3; n++){
		if(col < b->local_dims[n]){col = b->local_dims[n]-1;}
	}
		...
		if (b->ndims > n) {
			...
			switch(b->datatype){
			  case SDF_DATATYPE_REAL8:
				a.d = b->grids[n];
				for(i=0;i<b->local_dims[n]-1;i++){Data_set(data,n,i,a.d[i]);}
				break;
			  case SDF_DATATYPE_REAL4:
				a.f = b->grids[n];
				for(i=0;i<b->local_dims[n]-1;i++){Data_set(data,n,i,(double)(a.f[i]));}
				break;
			  default:
				break;
			}
