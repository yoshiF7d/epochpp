...
void Data_sort(Data data, int key){
	if(key < 0){key = data->column + key;}
	if(data->column <= key){fprintf(stderr,"key %d exceeds column length %d\n",key,data->column);}
	Data_sort_module(data,0,data->row-1,key);
}
...
void Data_sort_module(Data data, int i, int j, int key){
	...
	//fprintf(stderr,"Data_sort_module(%d,%d,%d)\n",i,j,key);
	pv = Data_sort_pivot(data,i,j,key);
	if(pv != -1){
		a = data->elem[pv][key]; 
		k = Data_sort_partition(data,a,i,j,key);
		Data_sort_module(data,i,k-1,key);
		Data_sort_module(data,k,j,key);
	}
	return;
}
...
int Data_sort_partition(Data data, double a, int i, int j, int key){
	...
	q = i; r = j;
	while(1){
		while(data->elem[q][key] < a) q++;
		while(data->elem[r][key] >= a) r--;
		if(q <= r){
			Data_rswap(data,q,r); q++; r--; 
		}
		else break;
	}
	return q;
	//fprintf(stderr,"Data_sort_partition\n");
}
...
int Data_sort_pivot(Data data, int i, int j, int key){
	...
	k = i + 1;
	while(k <= j){
		if(data->elem[i][key] == data->elem[k][key]){k++;}
		else{break;}
	}
	if(k>j){pv = -1;}
	else if(data->elem[i][key] >= data->elem[k][key]){pv = i;}
	else{pv = k;}
	//fprintf(stderr,"Data_sort_pivot\n");
	return pv;
}
